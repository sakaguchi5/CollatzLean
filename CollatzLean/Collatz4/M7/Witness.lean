...
structure M7Witness where
  ...
  g_lower :
    Collatz4.General.residualGMin (residualTwoExponent q) r ≤ gStar
  g_upper :
    gStar ≤ Collatz4.General.residualGMax (residualTwoExponent q) r
  recurrence :
    Collatz4.General.ResidualRecurrence r
...
theorem run_to_target (h : M7Witness) :
  ...
  exact Collatz4.General.run_eq_of_residual_recurrence h.recurrence
...
theorem hasM7Witness_reducesTo :
    Collatz4.General.ReducesTo HasM7Witness forwardProblem := by
  intro h
  rcases h with ⟨w⟩
