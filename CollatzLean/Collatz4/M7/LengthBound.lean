...
def candidateFamily : Collatz4.General.ArithmeticFamily :=
  Collatz4.General.intervalFamily candidateNLower candidateNUpper candidateStep
...
theorem candidateN_lower (i : Fin candidateCount) : 12 ≤ candidateN i := by
  have h := Collatz4.General.ArithmeticFamily.start_le_value candidateFamily i
...
theorem candidateN_upper (i : Fin candidateCount) : candidateN i ≤ 4368 := by
  ...
  simp [candidateN, candidateFamily, Collatz4.General.intervalFamily,
    Collatz4.General.ArithmeticFamily.value, candidateNLower,
    candidateNUpper, candidateRLower, candidateRUpper, candidateStep,
    targetTime, Collatz4.General.intervalCount] at *
  omega
...
theorem candidateN_even_mod (i : Fin candidateCount) : candidateN i % 2 = 0 := by
  simp [candidateN, candidateFamily, Collatz4.General.intervalFamily,
    Collatz4.General.ArithmeticFamily.value, candidateNLower,
    candidateNUpper, candidateRLower, candidateRUpper, candidateStep,
    targetTime, Collatz4.General.intervalCount, Nat.add_mod]
...
theorem exists_candidateN_of_bounds_even
  ...
  have hklt : k < candidateCount := by
    ...
    omega
  refine ⟨⟨k, hklt⟩, ?_⟩
  simp only [candidateN, General.ArithmeticFamily.value, candidateFamily, General.intervalFamily,
    candidateNLower,targetTime, candidateRUpper, Nat.reduceSub, candidateStep,
    General.intervalCount, candidateNUpper, candidateRLower,
