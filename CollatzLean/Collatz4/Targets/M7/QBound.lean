...
/-!
# Collatz4.Targets.M7.QBound

m=7 固有の包絡線データを、一般理論 `Collatz4.Finite` へ代入する特殊化。
...
def qEnvelopeAdmissible (q : ℕ) : Prop :=
  Collatz4.Finite.EnvelopeAdmissible gStar gEnvelope residualTwoExponent q
...
theorem q_le_1275_of_envelope {q : ℕ} (hq : qEnvelopeAdmissible q) : q ≤ 1275 := by
  exact Collatz4.Finite.q_le_of_envelope_boundary
    (cutoff := 1275)
    gEnvelope_monotone
