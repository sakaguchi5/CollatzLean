...
/-!
# Collatz3: record skeleton から導く局所 critical geometry

`CriticalRecordSkeleton` は global chord rank と roof-return だけを持つ。
このファイルでは、その skeleton に追加の算術仮定を与えたときに
各 block の local critical first-passage geometry を導く補題を置く。

重要な設計方針:

* ここでは `RecordFerrers` という完成済み structure を定義しない。
* `IsBestUpperWidth` は local criticality の必要条件とはみなさず、
  proper-prefix Beatty bound を一括して導くための十分条件としてだけ使う。
* interior block の minimal depth は roof-to-roof strict drop から theorem として導く。
* terminal block の minimal depth は現段階では `TerminalMinimalFrom` という
  独立 predicate として仮定し、後続の定理で除去できるかを調べる。
* primitive 性もここには保存しない。primitive は RecordView から strict skeleton へ
  上げる前半の bridge における十分条件候補である。

真の `RecordFerrers` の定義は、record-level tie と local carry の exact 条件を
確定した後に別途導入する。
-/
...
`IsBestUpperWidth` を必要条件ではなく、proper-prefix bound を一括して得る
十分条件として使う中心補題。
-/
...
これは現段階の補助 predicate であり、完成した `RecordFerrers` の field と確定したものではない。
-/
...
`IsBestUpperWidth` はここでは構成を保証する十分条件であり、
`LocalCriticalBlocksFrom` の必要条件とは主張しない。
interior minimal depth は `interior_localDepth_eq_criticalTwoDepth` から自動で出る。
-/
theorem localCriticalBlocksFrom_of_bestUpper
    {m : ℕ}
    {h : Profile m}
    (A : Admissible h)
    (Best : IsBestUpperWidth m) :
