import CollatzLean.Collatz3.Experimental.MechanicalInverse
...
/-!
# Collatz3 experimental: mechanical slope の exact convergent corridor
...
`Q/P < ρ < Qn/Pn`,  `Pn*Q + 1 = P*Qn`

upper orientation:

`Qn/Pn < ρ < Q/P`,  `P*Qn + 1 = Pn*Q`
...
theorem natFloor_eq_current_div_of_lowerFarey
    {ρ : ℝ}
    {P Q Pn Qn x : ℕ}
    (B : IsLowerFareyBracket ρ P Q Pn Qn)
    (hx : x < Pn) :
    ⌊(x : ℝ) * ρ⌋₊ = (x * Q) / P := by
  ...
  · have hxPos : 0 < x := Nat.pos_of_ne_zero hx0
    ...
    have hNonnegXRho : 0 ≤ (x : ℝ) * ρ := by positivity
    have hTransfer :=
...
theorem natFloor_eq_current_pred_div_of_upperFarey
    {ρ : ℝ}
    {P Q Pn Qn x : ℕ}
    (B : IsUpperFareyBracket ρ P Q Pn Qn)
    (hx : x < Pn) :
    ⌊(x : ℝ) * ρ⌋₊ = (x * Q - 1) / P := by
...
/-- lower Farey corridor を lower mechanical roof 本体へ持ち上げた exact formula。 -/
theorem eq_linear_add_current_div_of_lowerFarey
    {β : ℕ → ℕ}
    {a : ℕ}
    {ρ : ℝ}
...
/-- upper Farey corridor の `-1` correction を lower mechanical roof 本体へ持ち上げる。 -/
theorem eq_linear_add_current_pred_div_of_upperFarey
    {β : ℕ → ℕ}
    {a : ℕ}
    {ρ : ℝ}
    (M : IsLowerMechanicalRoof β a ρ)
...
/-- lower corridor の周期性を roof 全体へ持ち上げる。 -/
theorem add_currentP_eq_linearShift_add_Q_of_lowerFarey
    {β : ℕ → ℕ}
    {a : ℕ}
    {ρ : ℝ}
    (M : IsLowerMechanicalRoof β a ρ)
    ...
    (hRange : P + x < Pn) :
    β (P + x) = P * a + Q + β x := by
  rw [M.eq_linear_add_natFloor (P + x)]
  rw [M.eq_linear_add_natFloor x]
  rw [natFloor_add_currentP_eq_add_Q_of_lowerFarey B hRange]
  simp only [Nat.add_mul]
  omega

/-- upper corridor の positive-domain 周期性を roof 全体へ持ち上げる。 -/
theorem add_currentP_eq_linearShift_add_Q_of_upperFarey
    {β : ℕ → ℕ}
    {a : ℕ}
    {ρ : ℝ}
    (M : IsLowerMechanicalRoof β a ρ)
  ...
  rw [M.eq_linear_add_natFloor (P + x)]
  rw [M.eq_linear_add_natFloor x]
  rw [natFloor_add_currentP_eq_add_Q_of_upperFarey B hxPos hRange]
  simp only [Nat.add_mul]
  omega
