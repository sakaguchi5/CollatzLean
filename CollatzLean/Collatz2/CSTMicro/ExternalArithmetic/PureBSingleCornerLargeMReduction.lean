...
/-!
# Pure B single-corner: Stage 4 large-m reduction

このファイルは「large m を消して explicit finite range へ落とす」ための論理的な最終形を固定する。

strong Xi port に入るために必要なのは、single-corner packet `S` について
...
を満たすことと、actual left prefix の divisibility

  2^e | 3^(b-1) R_B + Psi(b-1)

である。

後者は Collatz actual trace から供給すべき exact prefix bridge、前者二つは
`m = b + width + rightLength` と Stage 1/3 の幅評価から供給する macro geometry である。

ここではこの二種類を一つの certificate にまとめる。certificate を具体的な `M0` で構成できれば、
`|E(B)|=1` を仮定した actual minimal B は自動的に `m < M0` へ落ちる。

重要: `PureBProfileObstruction.q` は bad side を表す `Nat` なので、この actual packet 内で
`q < 0` を結論にするのではなく、single-corner bad packet 自体を contradiction にする。
-/
...
/--
strong Stage-8 port に入るための purely geometric readiness。
-/
def StrongLeftReady
    {P : PureBProfileObstruction}
    (S : P.SingleExposedCornerRigidityPacket) : Prop :=
  1538 ≤ S.strongLeftPrecision ∧
...
`M0` は explicit finite cutoff。二つの field は:

* `leftPrefixDivisibility`: actual trace -> left critical Xi congruence。
* `strongReady_of_large`: `M0 <= m` なら left precision が strong range に入る。

この structure 自体は外部数論仮定ではない。どちらも現在の Collatz/Ferrers/Sturmian
geometry から証明して埋める target である。
-/
