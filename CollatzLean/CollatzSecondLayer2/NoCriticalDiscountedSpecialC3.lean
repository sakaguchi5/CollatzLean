...
/--
`(3/2)^q`型のscaled endpoint上界を持つSpecial C3 tower。
-/
...
/-- Polynomial型とdiscounted型をまとめたSpecial C3中央枝。 -/
...
/-- Polynomial Special C3を中央枝へ入れる。 -/
def toObstruction
    {O : OddOrbit}
    (R : PolynomialSpecialC3TowerData O) :
    SpecialC3ObstructionTowerData O :=
  .polynomial R
...
/-- large endpoint枝を排除できるwindow長閾値。 -/
noncomputable def discountedCutoff
  ...
  Classical.choose
    (polynomialBelowTwoPower R.discountedK R.discountedA)

/-- cutoff以後ではscaled多項式係数が`2^(q+1)`未満。 -/
...
noncomputable def discountedTailStart
  ...
  Classical.choose
    (T.selected_lengths_tend_to_infinity
      R.select R.select_strict R.discountedCutoff)

/-- tail開始以後のwindow長はcutoffを越える。 -/
...
/-- discounted出力が使う元first-deferred添字。 -/
...
/-- discounted出力が使う元first-crossing添字。 -/
noncomputable def discountedCrossingSelect
    ...
    (R : SuperPolynomialNoCriticalFirstDeferredTowerData T)
    (j : ℕ) : ℕ :=
  T.crossingIndex (R.discountedSelect j)
...
theorem discountedCrossingSelect_strict
  ...
  exact D.source.select_strict
    (T.select_strict
      (R.select_strict
        (Nat.add_lt_add_left hab R.discountedTailStart)))

/-- discounted tailの各terminalはactual Special C3。 -/
noncomputable def discountedSpecial
    ...
    SpecialC3At O
      ...
      (D.crossing.crossingLength
        (R.discountedCrossingSelect j)) := by
  classical
  let n := R.discountedSelect j
  let q := T.windowLength n
  have hqCutoff :
      R.discountedCutoff ≤ q := by
    have h :=
      R.discountedTailStart_spec
        (R.discountedTailStart + j)
        (by omega)
    simpa [n, q, discountedSelect] using h.le
  have hcoef :
      R.discountedK * (q + 1) ^ R.discountedA <
        2 ^ (q + 1) :=
    R.discountedCutoff_spec q hqCutoff
  have hscaled :=
    T.terminalEndpoint_scaled_le_of_noCritical
      n
      (R.noCritical (R.discountedTailStart + j))
  have hNonempty :
      Nonempty
        (SpecialC3At O
          ...
          (D.crossing.crossingLength
            (R.discountedCrossingSelect j))) := by
    rcases
        T.terminalEndpoint_large_or_special n with
      hlarge | hspecial
    · have hlower :
          2 ^ (q + 1) * 3 ^ q <
        ...
        calc
          2 ^ (q + 1) * 3 ^ q
            ...
            2 ^ q * (2 * 3 ^ q) := by
              ...
              ring
          _ <
            2 ^ q * T.terminalEndpoint n :=
              (Nat.mul_lt_mul_left
                (Nat.pow_pos (by omega))).2 hlarge
      have hupper :
          2 ^ q * T.terminalEndpoint n ≤
            (R.discountedK *
                (q + 1) ^ R.discountedA) *
      ...
      have hcoefScaled :
        ...
        (Nat.mul_lt_mul_right
          (Nat.pow_pos (by omega))).2 hcoef
      exfalso
      omega
    · simpa [
        discountedSelect,
        discountedCrossingSelect,
        discountedOffset,
        n,
        q,
...
/--
no-critical super-polynomial枝をdiscounted Special C3 towerへ送る。
long plateauはこの定理の補助的帰結であり、最終枝には残さない。
-/
